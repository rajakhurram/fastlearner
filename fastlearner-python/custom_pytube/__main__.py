...
import custom_pytube
import custom_pytube.exceptions as exceptions
from custom_pytube import extract, request
from custom_pytube import Stream, StreamQuery
from custom_pytube.helpers import install_proxy
from custom_pytube.innertube import InnerTube
from custom_pytube.metadata import YouTubeMetadata
from custom_pytube.monostate import Monostate
...
class YouTube:
    ...
    def js(self):
        ...
        if custom_pytube.__js_url__ != self.js_url:
            self._js = request.get(self.js_url)
            custom_pytube.__js__ = self._js
            custom_pytube.__js_url__ = self.js_url
        else:
            self._js = custom_pytube.__js__
    ...
    def fmt_streams(self):
        ...
        try:
            ...
            self._js = None
            self._js_url = None
            custom_pytube.__js__ = None
            custom_pytube.__js_url__ = None
            extract.apply_signature(stream_manifest, self.vid_info, self.js)
    ...
    @property
    def caption_tracks(self) -> List[custom_pytube.Caption]:
        ...
        return [custom_pytube.Caption(track) for track in raw_tracks]

    @property
    def captions(self) -> custom_pytube.CaptionQuery:
        """Interface to query caption tracks.

        :rtype: :class:`CaptionQuery <CaptionQuery>`.
        """
        return custom_pytube.CaptionQuery(self.caption_tracks)
