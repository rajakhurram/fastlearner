...
public class CouponController {
    ...
    public ResponseEntity<Message<Page<Coupon>>> fetchAll(Principal principal,
            @RequestParam(defaultValue = "0") Integer page,
            @RequestParam(defaultValue = "10") Integer size,
            @RequestParam(required = false) Boolean isActive) throws EntityNotFoundException {
        var m = service.fetchAll(page, size, isActive);
        return ResponseEntity.status(m.getStatus()).body(m);
    }

    @GetMapping(APIUrls.COUPON_FETCH_BY_ID)
    public ResponseEntity<Message<CouponRequest>> fetchById(@PathVariable Long id, Principal principal)
            throws EntityNotFoundException {
        var m = service.fetchById(id);
        return ResponseEntity.status(m.getStatus()).body(m);
    }

    @PatchMapping(APIUrls.COUPON_TOGGLE_STATUS)
    public ResponseEntity<Message<String>> toggleStatus(@PathVariable Long id, Principal principal)
            throws EntityNotFoundException {
        var m = service.toggleStatus(id);
        return ResponseEntity.status(m.getStatus()).body(m);
    }
    ...
    public ResponseEntity<Message<String>> create(@Valid @RequestBody CouponRequest request, Principal principal)
            throws InternalServerException, EntityNotFoundException, EntityAlreadyExistException {
