...
import com.vinncorp.fast_learner.services.excel.TestBulkQuestionExcelParser;
...
public class CourseController {
    ...
    private final TestBulkQuestionExcelParser testBulkQuestionExcelParser;
    ...
    @PostMapping(value = APIUrls.PARSE_TEST_BULK_QUESTIONS, consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public ResponseEntity<Message<BulkQuizQuestionsParseResponse>> parseTestBulkQuestionsFromExcel(
            @RequestPart("file") MultipartFile file) throws BadRequestException, IOException {
        BulkQuizQuestionsParseResponse data = testBulkQuestionExcelParser.parse(file);
        String message = data.getErrorCount() > 0
                ? "Parsed with " + data.getSuccessCount() + " valid question(s) and " + data.getErrorCount() + " row error(s)."
                : "Successfully parsed " + data.getSuccessCount() + " question(s).";
        var response = new Message<BulkQuizQuestionsParseResponse>()
