...
public class SubscriptionController {
    ...
    public ResponseEntity<Message<List<Subscription>>> fetchAllSubscription() throws EntityNotFoundException {
        Message<List<Subscription>> m = service.fetchAllSubscription();
        return ResponseEntity.status(m.getStatus()).body(m);
    }

    @GetMapping(APIUrls.GET_SUBSCRIPTION_BY_CYCLE)
    public ResponseEntity<Message<List<Subscription>>> fetchSubscriptionByCycle(@PathVariable String cycle) throws EntityNotFoundException {
        Message<List<Subscription>> m = service.fetchSubscriptionByCycle(cycle);
        return ResponseEntity.status(m.getStatus()).body(m);
    }
    ...
    @GetMapping(APIUrls.CURRENT_SUBSCRIPTION_PLAN_TYPE)
    public ResponseEntity<Message<String>> fetchCurrentSubscriptionPlanType(Principal principal) throws EntityNotFoundException {
        Message<String> m = subscribedUserService.fetchCurrentSubscriptionPlanType(principal.getName());
        return ResponseEntity.status(m.getStatus()).body(m);
    }
}
