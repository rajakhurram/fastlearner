...
public class QuizQuestion {
    ...
    public QuizQuestion(Long topicId, Long quizId, Long questionId, String questionText,String questionImageUrl, QuestionType questionType, String explanation) {
    ...
    public static List<QuizQuestion> from(List<RandomQuizProjection> randomQuizProjections){
       return randomQuizProjections.stream()
               .map(rq -> new QuizQuestion(rq.getTopicId(), rq.getQuizId(), rq.getQuestionId(), rq.getQuestionText(),rq.getAnswerImageUrl() ,QuestionType.valueOf(rq.getQuestionType()), rq.getExplanation())).collect(Collectors.toList());
    }
