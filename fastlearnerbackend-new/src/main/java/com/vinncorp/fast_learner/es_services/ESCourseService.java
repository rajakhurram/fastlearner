...
public class ESCourseService implements IESCourseService{
    ...
    private String SEARCH_RELATED_COURSES;
    ...
    private List<SearchRelatedCourses> fetchRelatedCourses(String input) {
        log.info("Fetching related courses by search keyword.");

        HttpHeaders httpHeaders = new HttpHeaders();
        httpHeaders.set("Authorization", AUTH_KEY);
        httpHeaders.setContentType(MediaType.APPLICATION_JSON);

        SearchRelatedCoursesRequest requestBody = new SearchRelatedCoursesRequest();
        requestBody.setQuery(input.toLowerCase().trim());
        HttpEntity<SearchRelatedCoursesRequest> entity = new HttpEntity<>(requestBody, httpHeaders);

        SearchRelatedCoursesResponse searchRelatedCoursesResponses = null;
        List<ListOfSearchRelatedCourses> listOfSearchRelatedCourses = null;
        try {
            ResponseEntity<SearchRelatedCoursesResponse> resp = restTemplate.exchange(SEARCH_RELATED_COURSES, HttpMethod.POST, entity, SearchRelatedCoursesResponse.class);

            if (resp.getStatusCode().is2xxSuccessful()) {
                searchRelatedCoursesResponses = resp.getBody();
                if (Objects.isNull(searchRelatedCoursesResponses) || searchRelatedCoursesResponses.getResults().isEmpty()) {
                    log.error("No related courses found by this course.");
                    return null;
                }
                listOfSearchRelatedCourses = searchRelatedCoursesResponses.getResults();
            } else {
                log.error("No related courses found by this course.");
                return null;
            }
        } catch (Exception e) {
            log.error("Related courses service is not working please contact support team.");
            return null;
        }
