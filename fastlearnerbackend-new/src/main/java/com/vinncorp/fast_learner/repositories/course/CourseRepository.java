...
public interface CourseRepository extends JpaRepository<Course, Long> {
    ...
    @Query(value = """
            SELECT DISTINCT c.id AS course_id, c.title AS course_title, cu.url AS course_url, c.description AS course_description, c.about,
                   ...
                   GROUP BY c.id, cu.url, c.description, c.about, c.thumbnail, c.preview_videourl, c.preview_video_vtt_content, 
                            c.prerequisite, c.course_outcome, c.last_mod_date, c.content_type, cl.name, us.id, us.full_name, up.profile_picture, 
                            up.profile_url, up.about_me, up.headline, c.course_duration_in_hours, mcr.max_rating, mcr.total_review, cc.name, 
                            e.id, fc.id, c.certificate_enabled, c.created_date, tc.total_students, c.meta_title, c.meta_description, c.meta_heading, 
    ...
    @Query(value = """
            SELECT 
                ...
                cc.name AS category,
                c.certificate_enabled AS has_certificate,
                c.meta_title,
                c.meta_description,
                c.meta_heading,
                c.course_type,
                c.price,
                COALESCE(qs.test_total_question, 0) AS test_total_question,
                CASE 
            ...
            -- Category, level, instructor, profile, etc.
            INNER JOIN course_category AS cc ON c.course_category_id = cc.id
            INNER JOIN users AS us ON us.id = c.created_by
            INNER JOIN course_level AS cl ON c.course_level = cl.id
            LEFT JOIN user_profile AS up ON up.created_by = us.id
            LEFT JOIN course_url AS cu ON cu.course_id = c.id AND cu.status = 'ACTIVE'
                        
            LEFT JOIN (
                SELECT 
                    c.id AS course_id,
                    SUM(q.random_question) AS test_total_question
                FROM course c
                JOIN section s ON s.course_id = c.id
                JOIN topic t ON t.section_id = s.id
                JOIN quiz q ON q.topic_id = t.id
                WHERE c.content_type = 'TEST'
                GROUP BY c.id
            ) AS qs ON qs.course_id = c.id
                        
            -- Optional enrollment join if needed elsewhere
            ...
            GROUP BY 
                c.id, c.title, cu.url, c.description, c.about, c.content_type,
                ...
                c.course_duration_in_hours, cc.name, c.certificate_enabled, 
                c.meta_title, c.meta_description, c.meta_heading, c.course_type, 
                c.price, mcr.max_rating, mcr.total_review, qs.test_total_question
    ...
    @Query(value = "SELECT count(DISTINCT c.id) as courses, COUNT(e.student_id) AS enrolled_students " +
            "FROM course c " +
            "LEFT JOIN enrollment e ON c.id = e.course_id " +
//            "AND e.student_id != :userId " +
            "WHERE c.instructor_id = :userId " +
            "AND c.course_status = 'PUBLISHED'", nativeQuery = true)
...
//                                             WHERE e2.course_id = c.id AND e2.student_id = :userId
//                                         )
//                                    THEN 'TRUE'
//                                    ELSE 'FALSE'
//                                END AS already_bought,
...
//
//            GROUP BY
//                c.id, c.title, cu.url, up.profile_url, cc.name, u.id, u.full_name, up.profile_picture, ta.tag_names,cc.description,cl.name,c.content_type
//            HAVING
//                SIMILARITY(CAST((c.title || ' ' || COALESCE(ta.tag_names, '') || ' ' || cc.description) AS TEXT), :courseDetail) > 0.2
...
//                                   Pageable pageable);
    ...
    @Query(value = """
            ...
            SELECT DISTINCT
                ...
                c.price,cl.name AS course_level,
                c.content_type,
                COALESCE(cq.test_total_question, 0) AS test_total_question,
                 (SELECT SUM(t1.duration_in_sec) FROM topic as t1 INNER JOIN section as s1 ON s1.id = t1.section_id INNER JOIN course as c1 ON c1.id = s1.course_id WHERE c1.id = c.id) as duration
            FROM course c
            INNER JOIN course_category cc ON c.course_category_id = cc.id
            INNER JOIN users u ON u.id = c.instructor_id
            ...
            INNER JOIN quiz as q ON q.topic_id = t.id
            LEFT JOIN (
                SELECT c1.id AS course_id, SUM(q2.random_question) AS test_total_question
                FROM course c1
                INNER JOIN section s1 ON s1.course_id = c1.id
                INNER JOIN topic t1 ON t1.section_id = s1.id
                INNER JOIN quiz q2 ON q2.topic_id = t1.id
                WHERE c1.content_type = 'TEST'
                GROUP BY c1.id
            ) AS cq ON cq.course_id = c.id
            ...
            GROUP BY
                c.id, c.title, cu.url, up.profile_url, cc.name, u.id, u.full_name, up.profile_picture, ta.tag_names,cc.description,cl.name,c.content_type, cq.test_total_question
            HAVING
                SIMILARITY(CAST((c.title || ' ' || COALESCE(ta.tag_names, '') || ' ' || cc.description) AS TEXT), :courseDetail) > 0.3
            """, nativeQuery = true, countQuery = """
                WITH TagAggregates AS (
                SELECT
                    c.id AS course_id,
...
SELECT DISTINCT
    ...
    @Query(value = """
            SELECT DISTINCT
             ...
             INNER JOIN course_url AS cu ON cu.course_id = c.id AND cu.status = 'ACTIVE'
             WHERE c.course_status = 'PUBLISHED' AND c.content_type = 'TEST'
             GROUP BY c.id, c.title, c.created_date, c.description, c.thumbnail, cl.name, cc.name,
                      mcr.max_rating, mcr.total_review, us.id, us.full_name, up.profile_picture,
                      up.about_me, up.headline, up.profile_url, cu.url, e.course_id, e.student_id
             ORDER BY max_rating DESC
            ...
            SELECT DISTINCT
              ...
              GROUP BY c.id, c.title, c.created_date, c.description, c.thumbnail, cl.name, cc.name,
                       ...
                       up.about_me, up.headline, up.profile_url, cu.url, e.course_id, e.student_id
              ORDER BY max_rating DESC""")
    Page<Tuple> findTest(@Param("userId") Long userId, Pageable pageable);
    ...
    @Query(value = """
            ...
            FROM course AS c
                 ...
                 LEFT JOIN user_profile AS up ON up.created_by = us.id
LEFT JOIN (
    SELECT c1.id AS course_id, SUM(q.random_question) AS test_total_question
    FROM course c1
    INNER JOIN section s1 ON s1.course_id = c1.id
    INNER JOIN topic t1 ON t1.section_id = s1.id
    INNER JOIN quiz q ON q.topic_id = t1.id
    WHERE c1.content_type = 'TEST'
    GROUP BY c1.id
) AS cq ON cq.course_id = c.id
