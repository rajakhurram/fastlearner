...
public interface InstructorSalesRepository extends JpaRepository<InstructorSales, Long> {
    ...
    @Transactional
    @Modifying
    @Query(value = "UPDATE instructor_sales SET stripe_account_id = :stripeAccountId WHERE status != 'PROCESSED' AND instructor_id = :userId", nativeQuery = true)
    void updateWithStripeAccountIdOfUnprocessedPayments(Long userId, String stripeAccountId);

    InstructorSales findByInstructorId(Long instructorId);
    ...
    WHERE instructor_id = :instructorId
      ...
      END;
    """, nativeQuery = true)
    Double fetchAvailableBalanceByPeriod(String period, Long instructorId);
    ...
    @Query(value = """
            ...
            WHERE instructor_id = :instructorId
            """, nativeQuery = true)
    Double fetchAllTimeSales(Long instructorId);
    ...
    @Query(value = """
            ...
            WHERE instructor_id = :instructorId
              AND status = 'PENDING'
            """, nativeQuery = true)
    Double fetchAllTimeAvailableBalance(Long instructorId);
}
