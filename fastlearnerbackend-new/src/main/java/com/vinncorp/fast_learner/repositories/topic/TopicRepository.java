...
public interface TopicRepository extends JpaRepository<Topic, Long> {
    ...
    @Query(value = """
            ...
            quiz_id, q.title, q.random_question,q.duration_in_minutes, q.passing_criteria,q.test_type, q.is_allowed_to_retake, q.generateaireport AS generate_ai_report,q.report_prompt AS report_prompt, art.id as article_id, art.content, ucp.id as progress_id, ucp.is_completed, ucp.seek_time
            FROM
                topic AS t
            ...
            GROUP BY
            	...
            	tt.name, v.id, v.filename, v.videourl, q.id, q.title, q.test_type, q.is_allowed_to_retake, q.generateaireport, q.report_prompt, art.id, art.content, ucp.id, ucp.is_completed, ucp.seek_time
            ORDER BY
                sequence_number ASC;
            """, nativeQuery = true)
