...
public interface UserCourseCompletionRepository extends JpaRepository<UserCourseCompletion, Long> {
    ...
    @Query(value = """
            ...
            WHERE
                ...
                AND e.student_id <> c.created_by
                AND (
                            :period = 'all_time'
                            OR (:period = 'monthly' AND DATE_TRUNC('month', e.enrolled_date) = DATE_TRUNC('month', CURRENT_DATE))
                            OR (:period = 'yearly' AND DATE_TRUNC('year', e.enrolled_date) = DATE_TRUNC('year', CURRENT_DATE))
                            OR (:period = 'previous_month' AND DATE_TRUNC('month', e.enrolled_date) = DATE_TRUNC('month', CURRENT_DATE - INTERVAL '1 month'))
                            OR (:period = 'previous_year' AND DATE_TRUNC('year', e.enrolled_date) = DATE_TRUNC('year', CURRENT_DATE - INTERVAL '1 year'))
                        )
            """, nativeQuery = true)
