...
public class CourseDetailResponse {
    ...
    public static CourseDetailResponse fromCourseData(Tuple courses) {
        ...
        return CourseDetailResponse.builder()
                ...
                .testTotalQuestion(
                        courses.getElements().stream().anyMatch(column -> "test_total_question".equals(column.getAlias()))
                                ? (courses.get("test_total_question") != null
                                ? (
                                courses.get("test_total_question") instanceof BigDecimal
                                        ? ((BigDecimal) courses.get("test_total_question")).longValue()
                                        : ((Number) courses.get("test_total_question")).longValue()
                        )
                                : 0
                        )
                                : 0
