...
import com.vinncorp.fast_learner.response.auth_net_checkout.CreateTransactionResponse;
...
public class AuthorizeNetCheckoutService implements IAuthorizeNetCheckoutService {
...
// ✅ CASE 1: Free Course
        if (courseAmount <= 0.0) {
            ...
            if (Objects.nonNull(chargePayment.getAffiliateUUID())) {
                Boolean isSaved = this.affiliateCourseService
                        .saveStudentOnboardingDetails(chargePayment.getAffiliateUUID(), chargePayment.getCourseId());
            ...
            // payout optional for free course (usually skip)
            payoutProcess(course, user, "COUPON_COURSE", chargePayment.getAffiliateUUID(), coupon);

            return new Message<String>()
                    .setStatus(HttpStatus.OK.value())
                    .setCode(HttpStatus.OK.name())
                    .setMessage("Successfully enrolled in free course: " + course.getTitle())
                    .setData("Successfully enrolled in free course: " + course.getTitle());
        }


// ✅ CASE 2: Paid Course
        response = chargeWithToken(chargePayment.getOpaqueData(), courseAmount, course.getId());
        ...
        if (response != null && response.getMessages().getResultCode() == MessageTypeEnum.OK) {
            ...
            payoutProcess(course, user,
                    response.getTransactionResponse().getTransId(),
                    chargePayment.getAffiliateUUID(),
                    coupon);

            return new Message<String>()
                    .setStatus(HttpStatus.OK.value())
                    .setCode(HttpStatus.OK.name())
                    .setMessage("Successfully charged $" + courseAmount + " for course: " + course.getTitle())
                    .setData("Successfully charged $" + courseAmount + " for course: " + course.getTitle());
        }
...
// ❌ Error handling
        ...
        if (response != null &&
            ...
            errorResp = "Payment failed. No response from payment gateway.";
        }
    ...
    public CreateTransactionResponse chargeWithToken(String opaqueData, double amount, Long courseId) throws InternalServerException {
        ...
        return restClient.makeRequest("/v1/checkout/", HttpMethod.POST, request, CreateTransactionResponse.class);
    }
}
