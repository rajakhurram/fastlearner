...
import com.vinncorp.fast_learner.validation.course.CourseTypeTransitionMessages;
import com.vinncorp.fast_learner.validation.course.CourseTypeTransitionValidator;
...
public class CourseService implements ICourseService {
    ...
    public void createCourseValidateForThirdParty(CreateCourseRequest request) throws CreateCourseValidationException, EntityNotFoundException {
        ...
        if (request.getCourseId() != null) {
            Optional<Course> existingCourseOptional = repo.findById(request.getCourseId());
            if (existingCourseOptional.isPresent()) {
                Course existingCourse = existingCourseOptional.get();
                if (CourseTypeTransitionValidator.isPremiumCourseEditBlocked(existingCourse)) {
                    error.put("courseType", CourseTypeTransitionMessages.PREMIUM_COURSE_NOT_EDITABLE);
                } else {
                    CourseType requestedEffectiveType = CourseTypeTransitionValidator.resolveEffectiveCourseType(request);
                    CourseTypeTransitionValidator.validateTransition(existingCourse, requestedEffectiveType)
                            .ifPresent(message -> error.put("courseType", message));
                }
            } else {
                error.put("courseId", "Course ID is required and must be valid.");
            }
        }
        ...
        if (request.getPrerequisite() == null || request.getPrerequisite().isEmpty()) {
            ...
            boolean hasBlank = request.getPrerequisite().stream()
                    .anyMatch(c -> c == null || c.trim().isEmpty());
                            ...
                            if (topic.getQuiz() != null) {
                                ...
                                if (topic.getQuiz().getQuestions().size() > 0) {
                                    for (CreateQuizQuestionRequest quizQuestion : topic.getQuiz().getQuestions()) {
                                        ...
                                        boolean isSurvey = topic.getQuiz().getTestType() != null
                                                && topic.getQuiz().getTestType().equals(TestType.SURVEY);
                                        if (quizQuestion.getAnswers().size() > 1) {
                                            ...
                                            for (CreateQuizQuestionAnswerRequest quizQuestionAnswer : quizQuestion.getAnswers()) {
                                                ...
                                                if (!isSurvey) {
                                                    ...
                                                    if (Boolean.TRUE.equals(quizQuestionAnswer.getIsCorrectAnswer())) {
                                                        isAnswerExist = true;
                                                    }
                                                }

                                            }
                                            if (!isSurvey && !isAnswerExist) {
                                                error.put("isCorrectAnswer", "Each question must have at least one correct answer");
                                            }
                                        } else {
    ...
    */
    private boolean validateAlphanumericString(String title) {
        ...
        String regex = "^(?=.*[a-zA-Z])[a-zA-Z0-9\\s!@#$%^&*()_+={}\\[\\]:;\"'<>,.?/\\\\|`~\\-]{10,60}$";
    ...
    public Message<CreateCourseRequest> createCourse(CreateCourseRequest request, String clientId, String email)
        ...
        if (Objects.isNull(request.getCourseId())) {
            ...
            existingCourse = repo.findById(request.getCourseId())
                    .orElseThrow(() -> new BadRequestException("Course id provided is not valid."));

            if (CourseTypeTransitionValidator.isPremiumCourseEditBlocked(existingCourse)) {
                throw new BadRequestException(CourseTypeTransitionMessages.PREMIUM_COURSE_NOT_EDITABLE);
            }

            CourseType requestedEffectiveType = CourseTypeTransitionValidator.resolveEffectiveCourseType(request);
            Optional<String> transitionError = CourseTypeTransitionValidator.validateTransition(existingCourse, requestedEffectiveType);
            if (transitionError.isPresent()) {
                throw new BadRequestException(transitionError.get());
            }
        }
        ...
        if (!isCreation && request.getCourseType().equalsIgnoreCase(String.valueOf(CourseType.PREMIUM_COURSE))){
            ...
            if (existingCourse != null
                    && existingCourse.getCourseType() != CourseType.PREMIUM_COURSE
                    && CourseTypeTransitionValidator.isFreeOrStandardType(existingCourse.getCourseType())) {
                boolean isPremiumCourseAvailable = this.premiumCourseValidationBySubscription(instructor);
                if (!isPremiumCourseAvailable) {
                    log.error("Premium course conversion failed for instructor: {}. Reason: Premium course subscription validation failed.", instructor.getId());
            ...
            if(Objects.nonNull(e.getTopics()) && !e.getTopics().isEmpty()){
                for (CreateTopicRequest topic : e.getTopics()) {
                    ...
                    Topic savedTopic = topicService.save(Topic.builder()
                            ...
                            .name(topic.getTitle())
                            .topicComprehensive(topic.getTopicComprehensive()==null?null:topic.getTopicComprehensive())
                            .section(section)
                            .topicType(topicType)
                            .durationInSec(
                    ...
                    if (Objects.nonNull(topic.getQuiz())) {
                        if (!Objects.isNull(topic.getQuiz().getQuestions()) && !topic.getQuiz().getQuestions().isEmpty()) {
                            Quiz quiz = quizService.save(Quiz.builder()
                                    ...
                                    .generateAIReport(topic.getQuiz().getGenerateAIReport()!=null?topic.getQuiz().getGenerateAIReport():false)
                                    .testType(topic.getQuiz().getTestType())
                                    //prompt null or empty it can only append normal prompt but also check prompt get from attempt side?
                                    .reportPrompt(topic.getQuiz().getReportPrompt() ==null||topic.getQuiz().getReportPrompt().isBlank()?null:appendPrompt(topic.getQuiz().getReportPrompt()))
                                    .randomQuestion(topic.getQuiz().getRandomQuestion())
                                    .isAllowedToRetake(topic.getQuiz().getIsAllowedToRetake())
                                    .build());
                            boolean isTestTypeAlreadySet = topic.getId() != null
                                    && quizService.findQuizTestTypeByTopicId(topic.getId()).isPresent();
                            for (CreateQuizQuestionRequest question : topic.getQuiz().getQuestions()) {
                                ...
                                QuizQuestion quizQuestion = quizQuestionService.save(QuizQuestion.builder()
                                        ...
                                        .questionImageUrl(question.getQuestionImageUrl()==null?null: question.getQuestionImageUrl())
                                ...
                                boolean isSurvey = TestType.SURVEY.equals(quiz.getTestType());
                                ...
                                if (quiz.getTestType()==null || quiz.getTestType().equals(TestType.TEST)) {
                                    validateAnswers(quizQuestion.getQuestionType(), question.getAnswers());
                                }


                                // Assuming a SURVEY question cannot have more than 5 options.
                                // Therefore, all existing options for a question are deleted, when creating or switching the test type to SURVEY.
                                if (isTestTypeAlreadySet) {
                                    quizQuestionAnswerService.deleteAnswersByQuestionId(question.getId());
                                }
                                for (CreateQuizQuestionAnswerRequest answer : question.getAnswers()) {
                                    quizQuestionAnswerService.save(
                                            QuizQuestionAnwser.builder()
                                                    .id(answer.getId())
                                                    .delete(answer.getDelete())
                                                    .quizQuestion(quizQuestion)
                                                    .answer(answer.getAnswerText())
                                                    .answerImageUrl(answer.getAnswerImageUrl() == null || answer.getAnswerImageUrl().isBlank() ? null : answer.getAnswerImageUrl())
                                                    .isCorrectAnswer(Boolean.TRUE.equals(answer.getIsCorrectAnswer()))
                                                    .build()
                                    );
                                }
                            }
    ...
    public static String appendPrompt(String reportPrompt) {
            return Prompts.AI_REPORT_INSTRUCTOR_PROMPT_APPEND + "\nInstructor emphasis: " + reportPrompt;
    ...
    private Course savingCourse(CreateCourseRequest request, User instructor, CourseLevel courseLevel, CourseCategory courseCategory) throws InternalServerException {
        try {
            ...
            CourseType existingCourseType = null;
            boolean pricingLocked = false;
            boolean premiumConversionUsed = false;
            if (Objects.nonNull(request.getCourseId())) {
                ...
                existingCourseType = course.getCourseType();
                pricingLocked = Boolean.TRUE.equals(course.getPricingLocked());
                premiumConversionUsed = Boolean.TRUE.equals(course.getPremiumConversionUsed());
            }
            ...
            if (courseType == CourseType.PREMIUM_COURSE) {
                pricingLocked = true;
                if (existingCourseType != null && CourseTypeTransitionValidator.isFreeOrStandardType(existingCourseType)) {
                    premiumConversionUsed = true;
                }
            }
            ...
            course = Course.builder()
                    ...
                    .premiumConversionUsed(premiumConversionUsed)
...
//            }
    ...
    public Message<RelatedCoursesResponse> getRelatedCoursesByPagination(RelatedCoursesRequest request, String email) throws EntityNotFoundException {
        ...
        Long userId = (user != null) ? user.getId() : null;
        // FETCH related courses from python service
        ...
        String categoryDesc = course.getCourseCategory().getDescription() != null
                ? course.getCourseCategory().getDescription()
                : "";

        String courseDetail = course.getTitle() + " " + course.getCourseCategory().getName() + " " + tagStringify;
        log.info("courseDetail being passed: '{}'", courseDetail);

        Page<Tuple> pagedData = repo.findRelatedCourses(
                request.getCourseId(),
                course.getTitle() + " " + course.getCourseCategory().getName() + " " + categoryDesc + " " + tagStringify, userId,
                PageRequest.of(request.getPageNo(), request.getPageSize()));
...
// Manually set the parent fields (previousPage, currentPage, nextPage)
    ...
    @Override
    public Message<CourseDetailByPaginatedResponse> getAllTest(Pageable pageable, Principal principal) {
        List<CourseDetailResponse> courseDetailResponse = new ArrayList<>();
        Page<Tuple> data;
        try {
            User user = null;
            ...
            data = repo.findTest(userId, pageable);
            log.info("Successfully fetched Tests data");
            if (data != null && data.hasContent()) {
                log.info("Mapping test data");
                courseDetailResponse = data.stream().map(CourseDetailResponse::fromCourseData).collect(Collectors.toList());
            } else {
                log.info("Test courses not found");
            }

        } catch (Exception e) {
            log.warn("Failed to fetch tests. Reason: {}", e.getMessage());
            return new Message<CourseDetailByPaginatedResponse>()
                    .setStatus(HttpStatus.INTERNAL_SERVER_ERROR.value())
                    .setCode(HttpStatus.INTERNAL_SERVER_ERROR.name())
                    .setMessage("An error occurred while fetching tests.");
        }
        ...
        return new Message<CourseDetailByPaginatedResponse>()
                .setData(response)
                .setMessage("All tests fetched successfully.")
                .setStatus(HttpStatus.OK.value())
                .setCode(HttpStatus.OK.toString());
    }
    @Override
