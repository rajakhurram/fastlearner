...
public class DashboardService implements IDashboardService{
    ...
    public Message<DashboardStatsResponse> fetchStats(String filterBy, String email) throws EntityNotFoundException, BadRequestException {
        ...
        if(
            ...
            && !StringUtils.equalsIgnoreCase(filterBy, "ALL_TIME"))) {
            throw new BadRequestException("Filter should be All time, This month, Last month, This year or Last year only.");
        }
        ...
        dashboardStatsResponse.setRevenue(
                instructorSalesService.fetchMonthlyOrYearlySales(filterBy.toLowerCase(), user.getId()));
        dashboardStatsResponse.setAvailableBalance(
                instructorSalesService.fetchAvailableBalance(filterBy.toLowerCase(), user.getId()));
