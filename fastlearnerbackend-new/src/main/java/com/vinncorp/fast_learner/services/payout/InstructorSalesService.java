...
public class InstructorSalesService implements IInstructorSalesService{
    ...
    public Double fetchMonthlyOrYearlySales(String period, Long instructorId) {
        ...
        if (period.equalsIgnoreCase("ALL_TIME")) {
            return repo.fetchAllTimeSales(instructorId);
        }
    ...
    @Override
    public void updateWithStripeAccountIdOfUnprocessedPayments(Long userId, String stripeAccountId) {
        log.info("Update the stripe account id of unprocessed payments...");
        repo.updateWithStripeAccountIdOfUnprocessedPayments(userId, stripeAccountId);
    }

    @Override
    public Double fetchAvailableBalance(String period, Long instructorId) {
        log.info("Fetching available balance by " + period.toLowerCase() + " for instructor: " + instructorId);
        if (period.equalsIgnoreCase("ALL_TIME")) {
            return repo.fetchAllTimeAvailableBalance(instructorId);
        }
        return repo.fetchAvailableBalanceByPeriod(period.toLowerCase(), instructorId);
    }

    public InstructorSales findInstructorByUserId(Long instructorId) {
        InstructorSales instructorSales=  repo.findByInstructorId(instructorId);
        if (instructorSales!=null){
            return instructorSales;
        }
        log.info("Instructor sales not found");
        return null;
    }
