    ...
    public void executePayout(Course course, User user, String transId, String uuid, Coupon coupon) throws BadRequestException, InternalServerException {
        try {
            ...
            if (uuid != null) {
                ...
                InstructorAffiliate instructorAffiliate = instructorAffiliateRepository.findByAffiliateUserAndAffiliateUuid(instructorAffiliateId, uuid);
            ...
            saveInstructorSales(course.getInstructor().getId(), payoutAmount);
            service.save(premiumCoursePayoutHistory);
        }catch (Exception ex){
            ex.printStackTrace();
            log.info(ex.getMessage());
            throw new RuntimeException("============= Some thing went wrong while executing Payout! ===============");
        }
    }

    private void saveInstructorSales(Long instructorId,Double amount){
        InstructorSales instructorSales =new InstructorSales();
        instructorSales.setInstructorId(instructorId);
        instructorSales.setStatus(PayoutStatus.PENDING);
        instructorSales.setCreationDate(new Date());
        instructorSales.setTotalSales(amount);
        instructorSalesService.save(instructorSales);
