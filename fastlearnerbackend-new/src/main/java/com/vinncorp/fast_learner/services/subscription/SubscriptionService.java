...
public class SubscriptionService implements ISubscriptionService{
    ...
    @Override
    public Message<List<Subscription>> fetchAllSubscription() throws EntityNotFoundException {
    ...
    @Override
    public Message<List<Subscription>> fetchSubscriptionByCycle(String cycle) throws EntityNotFoundException {
        log.info("Fetching all subscriptions by cycle.");
        List<Subscription> subscriptions = repo.findAllByDuration(cycle);
        if (CollectionUtils.isEmpty(subscriptions)) {
            log.error("No subscription cycle found.");
            throw new EntityNotFoundException("No subscription cycle found.");
        }
        log.info("All subscription by cycle is fetched successfully.");
        return new Message<List<Subscription>>()
                .setStatus(HttpStatus.OK.value())
                .setCode(HttpStatus.OK.toString())
                .setMessage("All subscription by cycle is fetched successfully.")
                .setData(subscriptions);
    }
