...
@SpringBootTest
public class UpgradeProcessIntegrationTest {

    @Qualifier(value = "upgrade")
    @Autowired
    private SubscriptionStrategyService upgradeProcess;
    ...
    public void testUpgradeFromFreeToPaid_whenProvidedDataIsValid() throws EntityNotFoundException, InternalServerException, BadRequestException {
        ...
        var subscription = subscriptionService.findBySubscriptionId(5L).getData();
        var subscribedUser = subscribedUserService.findByUser(user.getEmail());
        var m = upgradeProcess.processSubscription(SubscriptionTestData.subscriptionRequest(), subscribedUser, subscription, user, null);
