...
public class QuestionController {
    ...
    @GetMapping(APIUrls.GET_QUESTIONS_AND_ANSWERS_BY_TOPIC)
    public ResponseEntity<Message<QuizQuestionAnswer>> getQuestionsAndAnswersByTopic(@RequestParam("topicId") Long topicId,
                                                                                     @RequestParam(value = "forStudent", required = false) Boolean forStudent,
        ...
        var m = quizQuestionAnswerService.fetchPaginatedQuestionAndAnswersByTopicId(topicId, false, false, PageRequest.of(pageNo, 10), principal.getName());
        return ResponseEntity.status(m.getStatus()).body(m);
    }
