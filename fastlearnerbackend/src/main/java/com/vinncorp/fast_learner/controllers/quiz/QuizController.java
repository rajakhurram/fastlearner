...
public class QuizController {
    ...
    public ResponseEntity<Message<QuizAnswerResponse>> validateAnswering(
            @RequestBody List<ValidationAnswerRequest> validationAnswerRequests,Principal principal) throws BadRequestException, InternalServerException {
        var m = quizQuestionAnswerService.validateAnswers(validationAnswerRequests, principal.getName());
        return ResponseEntity.status(m.getStatus()).body(m);
    }

    @PostMapping(APIUrls.PREVIEW_REPORT)
    public String previewReport(
            @RequestBody PreviewReport previewReport, Principal principal
    ) throws BadRequestException, InternalServerException {

        return quizQuestionAnswerService.previewReport(previewReport, principal.getName());
