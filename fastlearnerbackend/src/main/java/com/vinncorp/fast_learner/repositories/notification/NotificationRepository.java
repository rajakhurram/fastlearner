...
public interface NotificationRepository extends JpaRepository<Notification, Long> {
    ...
    @Query(
            value = "SELECT CASE WHEN COUNT(*) > 0 THEN true ELSE false END " +
                    ...
                    "AND :receiverId = ANY(n.receiver_ids) " +
                    "AND n.fullname = :senderName " +
                    "AND n.type = :type " +
                    "AND n.is_read = :isRead " +
                    "AND n.redirect_id = :redirectId " +
                    "AND DATE(n.creation_date) = CURRENT_DATE",
            nativeQuery = true
    )
    boolean existsNotificationToday(
            ...
            @Param("redirectId") Long redirectId
    );
}
