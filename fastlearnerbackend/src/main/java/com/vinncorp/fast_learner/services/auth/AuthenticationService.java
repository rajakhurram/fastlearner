...
//        ISocialLoginService loginService = SocialLoginFactory.build(AuthProvider.valueOf(provider), restTemplate, userService, roleService, jwtUtils, userProfileService);
        return googleLoginService.login(token, clientId, userName);
    }
    ...
    public TokenResponse localLogin(LocalAuthRequest request) throws BadRequestException, ForbiddenException,EntityNotFoundException {
        ...
        SubscribedUser subscribedUser = this.subscribedUserService.findByUser(request.getEmail().toLowerCase());
        ...
        return TokenResponse.builder()
                ...
                .age(user.getAge())
                .subscriptionPlanType(subscribedUser.getSubscription().getPlanType().name())
                .build();
    }
    ...
    @Override
    public TokenResponse localRegister(LocalRegisterRequest request) throws InternalServerException, EntityAlreadyExistException, EntityNotFoundException, BadRequestException {
        log.info("Logging into system via local email and password.");
        User savedUser = null;
        try {
...
//        }
        ...
        // TODO create free subscription when the user registers
...
//        subscriptionContextService.process(null, null, freeSubscription, savedUser, null);
...
//                    .getPassword()))
    ...
    @Override
    public TokenResponse verifyAuthenticationOtp(String email, int otp) throws BadRequestException, EntityAlreadyExistException, InternalServerException, EntityNotFoundException {
