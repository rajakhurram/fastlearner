...
public class AuthorizeNetSubscriptionService implements IAuthorizeNetSubscriptionService {
    ...
    public Message<String> create(SubscriptionRequest subscriptionRequest, String email)
        ...
        log.info("Creating authorize.net subscription for user " + email);
        ...
        subscriptionRequest.setPrevAuthSubscriptionId(subscribedUser==null?null:subscribedUser.getAuthorizeNetSubscriptionId());
    ...
    public boolean validationOnChangingPlan(SubscribedUser subscribedUser) throws EntityNotFoundException, BadRequestException {
        ...
        log.info("Retrieving latest transaction history for subscription ID: {} with ACTIVE status",
                subscribedUser.getAuthorizeNetSubscriptionId());

        TransactionHistory transactionHistory = transactionHistoryService.
                findByLatestTransactionHistoryBySubsIdAndStatus(subscribedUser.getAuthorizeNetSubscriptionId(), GenericStatus.ACTIVE);

        if (transactionHistory == null) {
            log.info("No active transaction history found for subscription ID: {}", subscribedUser.getAuthorizeNetSubscriptionId());
            return true;
        } else {
            log.info("Active transaction history found for subscription ID: {}", subscribedUser.getAuthorizeNetSubscriptionId());
    ...
    public Message<List<BillingHistoryResponse>> getBillingHistory(BillingHistoryRequest request, String email) throws EntityNotFoundException, InternalServerException, BadRequestException {
        ...
        if (Objects.isNull(subscribedUser.getAuthorizeNetSubscriptionId())) {
            log.error("ERROR: Customer profile id not exist for user " + email);
            throw new BadRequestException("Customer profile id not exists");
        }

        if (subscribedUser.getAuthorizeNetSubscriptionId().equals("0000000") && subscribedUser.getCustomerProfileId().equals("000000000")) {
        ...
        GetSubscriptionResponse transactions = authNetAdditionalSubscriptionService.getSubscriptionById(subscribedUser.getAuthorizeNetSubscriptionId());
    ...
    public Message<String> updateSubscription(Long subscribedUserProfileId, String email) throws EntityNotFoundException, BadRequestException, InternalServerException {
        ...
        SubscribedUser subscribedUser = subscribedUserService.findByUser(email);
        if (Objects.isNull(subscribedUser.getCustomerProfileId()) || Objects.isNull(subscribedUser.getAuthorizeNetSubscriptionId()))
            throw new BadRequestException("Current subscription doesn't have a authorized subscription.");

        // Update the current subscription
        var customerProfileData = authNetAdditionalSubscriptionService.updateAuthorizeNetSubscription(email, subscribedUser.getAuthorizeNetSubscriptionId(),
    ...
    public Message<String> freeSignUpSubscriptionNew(Long subscriptionId, User user) throws EntityNotFoundException, InternalServerException  {
        Subscription sub = subscriptionService.findBySubscriptionId(subscriptionId).getData();
        return createProces.createFreeSubscription(sub, user);
    }
}
