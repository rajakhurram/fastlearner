package com.vinncorp.fast_learner.services.authorize_net.payment_profile;
...
public class PaymentProfileService implements IPaymentProfileService {
    ...
    public CreateCustomerPaymentProfileResponse createCustomerPaymentProfile(String customerProfileId, PaymentProfileDetailRequest detail, String email) throws InternalServerException {
        log.info("Creating the auth net customer profile with id " + customerProfileId);
        ...
        createCustProfileRequest.setAddress(detail.getAddress());
        createCustProfileRequest.setZipCode(detail.getZipCode());
        createCustProfileRequest.setCountryCode(detail.getCountryCode());


        CreateCustomerPaymentProfileResponse response = restClient.makeRequest(
                "/v1/customer-payment-profile/save", HttpMethod.POST, createCustProfileRequest, CreateCustomerPaymentProfileResponse.class);
    ...
    public void updateCustomerPaymentProfile(String customerProfileId, String customerPaymentProfileId, PaymentProfileDetailRequest detail, String email) throws InternalServerException {
        log.info("Updating the auth net customer payment profile with id  " + customerPaymentProfileId);
        ...
        updateCustomerProfileReq.setExpiryDate(detail.getExpiryMonth() + detail.getExpiryYear());
        ...
        updateCustomerProfileReq.setZipCode(detail.getZipCode());
        updateCustomerProfileReq.setCountryCode(detail.getCountryCode());
        updateCustomerProfileReq.setAddress(detail.getAddress());
        updateCustomerProfileReq.setCity(detail.getCity());
        updateCustomerProfileReq.setState(detail.getState());



        UpdateCustomerPaymentProfileResponse response = restClient.makeRequest(
                "/v1/customer-payment-profile/update", HttpMethod.PUT, updateCustomerProfileReq, UpdateCustomerPaymentProfileResponse.class);
    ...
    public GetCustomerPaymentProfileResponse getCustomerPaymentProfile(String customerProfileId, String customerPaymentProfileId) throws InternalServerException {
        log.info("Fetching the customer payment profile from auth net with id " + customerPaymentProfileId);

        GetCustomerPaymentProfileResponse response = restClient.makeRequest(
                "/v1/customer-payment-profile/get?customerProfileId=" + customerProfileId + "&customerPaymentProfileId=" + customerPaymentProfileId,
    ...
    public GetCustomerProfileResponse getCustomerPaymentProfileList(String customerProfileId) throws InternalServerException {
        log.info("Fetching the customer payment profile list from auth net with id " + customerProfileId);

        GetCustomerProfileResponse response = restClient.makeRequest(
                "/v1/customer-profile/get/list?customerProfileId=" + customerProfileId, HttpMethod.GET, null, GetCustomerProfileResponse.class);
    ...
    public void deleteCustomerPaymentProfile(String customerProfileId, String customerPaymentProfileId, SubscribedUser subscribedUser) throws InternalServerException {
        log.info("Deleting the customer payment profile list from auth net with id " + customerPaymentProfileId);
        ...
        DeleteCustomerPaymentProfileResponse response = restClient.makeRequest(
                "/v1/customer-payment-profile/delete?customerPaymentProfileId=" + customerPaymentProfileId + "&customerProfileId=" + customerProfileId, HttpMethod.DELETE, null, DeleteCustomerPaymentProfileResponse.class);
    ...
    public GetTransactionDetailsResponse getTransactionDetail(String transactionId) throws InternalServerException {
        log.info("Fetching the transaction detail from auth net api with id " + transactionId);

        GetTransactionDetailsResponse getResponse = restClient.makeRequest(
                "/v1/transaction/detail?transactionId=" + transactionId, HttpMethod.GET, null, GetTransactionDetailsResponse.class);
        ...
        if (Objects.isNull(getResponse)) {
            ...
            throw new InternalServerException("Authorize.Net Subscription: " + getResponse.getMessages().getMessage().get(0).getText());
        }
    ...
    public GetSubscriptionResponse getSubscription(String subscriptionId) throws InternalServerException {
        log.info("Fetching the subscription detail from auth net api with id " + subscriptionId);
    ...
    private static String convertDateToAuthNetDateFormat(String inputDate) {
    ...
    public CustomerAndPaymentId createCustomerProfile(String email, SubscriptionRequest requestDTO) throws InternalServerException {
        ...
        createCustProfileReq.setAddress(requestDTO.getPaymentDetail().getAddress());
        createCustProfileReq.setCountryCode(requestDTO.getPaymentDetail().getCountryCode());
        createCustProfileReq.setZipCode(requestDTO.getPaymentDetail().getZipCode());

        CreateCustomerProfileResponse response = restClient.makeRequest(
                "/v1/customer-profile/create", HttpMethod.POST, createCustProfileReq, CreateCustomerProfileResponse.class);
    ...
    public void deleteCustomerProfileById(String profileId) throws BadRequestException {
        log.info("Deleting a customer profile id from auth net server...");

        try {
            DeleteCustomerProfileResponse response = restClient.makeRequest(
                    "/v1/customer-profile/delete?customerProfileId=" + profileId, HttpMethod.DELETE,
            ...
            if (response != null) {
                if (response.getMessages().getResultCode() == MessageTypeEnum.OK) {
                    ...
                    log.warn("Customer with ID: " + profileId + "doesn't deleted from auth net server.");
