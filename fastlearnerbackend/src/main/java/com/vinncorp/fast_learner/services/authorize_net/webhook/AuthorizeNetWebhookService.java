...
public class AuthorizeNetWebhookService implements IAuthorizeNetWebhookService {
    ...
    @Override
    public void subscriptionTermination(AuthorizeNetWebhookRequest request) throws EntityNotFoundException, InternalServerException {
        ...
        try {
            subscribedUser = subscribedUserService.findBySubscribedId("" + request.getPayload().getId());
        } catch (Exception e) {
            log.info("Exception while finding subscribed user: {}", e.getMessage());
            return;
        }

        if (subscribedUser == null) {
            log.info("Subscribed user not found with subscriptionId: {}", request.getPayload().getId());
            return;
        }

        Subscription prevSubscription = subscribedUser.getSubscription();
        if (subscribedUser != null && subscribedUser.getEndDate() == null) {
            ...
            instructorPerformanceInsightService.notifyToUserOnNewSubscription(prevSubscription.getDuration() == 1 ? "Monthly" : "Yearly", subscribedUser.getUser().getId());
