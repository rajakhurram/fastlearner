...
public class ExcelExportService {
    ...
    public byte[] exportPdfForPersonalityTest(List<QuestionEvaluationResponse> responses) throws IOException {
        ByteArrayOutputStream out = new ByteArrayOutputStream();
        ...
        for (QuestionEvaluationResponse q : responses) {
            ...
            // Options with dynamic letters
            if (q.getCorrectAnswers() != null && !q.getCorrectAnswers().isEmpty()) {
                char optionLetter = 'A';
                for (String option : q.getCorrectAnswers()) {
                    document.add(new Paragraph(optionLetter + ". " + option)
                            .setFontSize(11)
                            .setMarginLeft(10));
                    optionLetter++;
                }
            ...
            if (q.getStudentAnswers() != null && !q.getStudentAnswers().isEmpty()) {
                document.add(new Paragraph("Selected Answer: " + String.join(", ", q.getStudentAnswers()))
                        .setFontSize(11)
                        .setMarginLeft(10));
            }
