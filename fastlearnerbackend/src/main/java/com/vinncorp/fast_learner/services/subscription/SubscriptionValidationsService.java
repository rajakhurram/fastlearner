...
public class SubscriptionValidationsService implements ISubscriptionValidationsService {
    ...
    @Override
    public SubscriptionValidations getBySubscriptionIdAndValidationName(Long subscriptionId, String validationName) {
        log.info("Attempting to find SubscriptionValidation with validationName: {}, subscriptionId: {}", validationName, subscriptionId);

        return repo.findByValidationNameAndSubscriptionIdAndIsActive(validationName, subscriptionId, true)
                .orElse(null);
    }
}
