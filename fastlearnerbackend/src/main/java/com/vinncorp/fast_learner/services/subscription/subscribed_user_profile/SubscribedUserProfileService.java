...
import com.vinncorp.fast_learner.dtos.authorize_net.payment_profile.PaymentProfileDetailResponse;
...
public class SubscribedUserProfileService implements ISubscribedUserProfileService {
    ...
    public Message<String> deleteSubscribedUserProfile(Long id) throws EntityNotFoundException, InternalServerException, BadRequestException {
        ...
        deletingPaymentProfileFromAuthNetServer(subscribedUserProfile);
    ...
    public void deletingPaymentProfileFromAuthNetServer(SubscribedUserProfile subscribedUserProfile) throws InternalServerException, BadRequestException {
        // Check if the user has not a cancelled subscription and has authorize net subscription id other than static id i.e "0000000"
        if(Objects.isNull(subscribedUserProfile.getSubscribedUser().getEndDate()) &&
                ( Objects.nonNull(subscribedUserProfile.getSubscribedUser().getAuthorizeNetSubscriptionId()) &&
                        !subscribedUserProfile.getSubscribedUser().getAuthorizeNetSubscriptionId().equals("0000000"))) {

            GetSubscriptionResponse subscriptionResponse = paymentProfileService.getSubscription(subscribedUserProfile.getSubscribedUser().getAuthorizeNetSubscriptionId());
    ...
    public void deleteBySubscribedUserId(SubscribedUser subscribedUser) {
        ...
        if(!CollectionUtils.isEmpty(subscribedUserProfiles)) {
            subscribedUserProfiles.forEach(e -> {
                try {
                    deletingPaymentProfileFromAuthNetServer(e);
