...
public class TransactionHistoryService implements ITransactionHistoryService {
    ...
    @Override
    public TransactionHistory findByAuthSubscriptionId(String authorizeNetSubscriptionId) throws EntityNotFoundException {
        if (authorizeNetSubscriptionId != null && !authorizeNetSubscriptionId.isEmpty()) {
            TransactionHistory transactionHistory = transactionHistoryRepository.findByAuthSubscriptionId(authorizeNetSubscriptionId);
            if (transactionHistory == null) {
                throw new EntityNotFoundException("Authorize net subscription not found");
            }
    ...
    @Override
    public TransactionHistory findByLatestTransactionHistoryBySubsIdAndStatus(String authorizeNetSubscriptionId, GenericStatus genericStatus) throws EntityNotFoundException {
    ...
    @Override
    public TransactionHistory findByLatestTransactionHistoryBySubsIdAndStatusAndSubscriptionStatus(String authorizeNetSubscriptionId,
        ...
        TransactionHistory transactionHistory = transactionHistoryRepository
                .findFirstByStatusAndAuthSubscriptionIdAndSubscriptionStatusOrderByCreationAtDesc(
                        genericStatus, authorizeNetSubscriptionId, subscriptionStatus);
        if (transactionHistory == null) {
            throw new EntityNotFoundException("Authorize net subscription not found");
        }
        return transactionHistory;
    ...
    public TransactionHistory fetchCurrentSubscription(Long studentId) throws EntityNotFoundException {
        ...
        var transactionHistory = transactionHistoryRepository.fetchCurrentSubscription(studentId)
                .orElseThrow(() -> new EntityNotFoundException("No subscription present for this student."));

        Hibernate.initialize(transactionHistory.getSubscription());
        return transactionHistory;
    }
    ...
    public ResponseEntity<byte[]> downloadInvoiceByTransactionId(Long transactionId) throws EntityNotFoundException {
        ...
        try {
            ...
            var subscription = transactionHistory.getSubscription();
            String subscribedPlanWithDuration = getPlanWithDuration(subscription.getName(), subscription.getDuration());
            String htmlContent = InvoiceTemplate.generateInvoiceTemplate(
                    subscribedPlanWithDuration, // Premium Instructor Plan - Monthly or Yearly
...
//            throw new EntityNotFoundException("Authorize net subscription not found");
    ...
    private String getPlanWithDuration(String subscriptionName, int duration) {
        return switch (duration) {
            case 0 -> subscriptionName + " - " + "Forever";
            case 1 -> subscriptionName + " - " + "Monthly";
            case 12 -> subscriptionName + " - " + "Yearly";
            default -> "";
        };
    }
}
