...
public class CancelProcessIntegrationTest {
    ...
    void setUp() throws EntityNotFoundException, InternalServerException {
        ...
        subscribedUser.setAuthorizeNetSubscriptionId("67890");
        ...
        doNothing().when(authNetAdditionalSubscriptionService).cancelAuthorizeNetSubscription(anyString());
    }
...
//        assertTrue(exception.getMessage().contains("plan already cancelled"));
    ...
    @Test
    public void testCancelSubscriptionProcess_whenTheSubscriptionAcquiredToday() throws InternalServerException, BadRequestException, EntityNotFoundException {
        // Setting subscribedUser's start date to today's date
        ...
        doNothing().when(authNetAdditionalSubscriptionService).cancelAuthorizeNetSubscription(anyString());
        when(transactionHistoryService.findByLatestTransactionHistoryBySubsIdAndStatus(any(), any())).thenReturn(transactionHistory);
        when(transactionHistoryService.save(any())).thenReturn(transactionHistory);
        when(subscriptionService.findBySubscriptionId(anyLong())).thenReturn(new Message<>(HttpStatus.OK.value(), "OK", "Success", nextSubscription));
        when(subscribedUserService.save(any())).thenReturn(subscribedUser);
        ...
        verify(transactionHistoryService, times(2)).save(any());
        verify(subscribedUserService, times(1)).save(any());
    }
}
