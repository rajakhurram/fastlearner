...
export class GraderClassesComponent {
  ...
  editClassValue(aClass?: any) {
    if (
      ...
      aClass?.editingValue?.trim()?.length > 0
    ) {
    ...
    this._aiGraderService.editClass(classId, editValue)?.subscribe({
      next: (response: any) => {
        ...
        if (
          ...
          if (targetClass) {
              targetClass.name = updatedClass.name; // <-- use backend name
            }
          setTimeout(() => {
  ...
  editValue(event?: any) {
    if(event?.value?.trim()?.length > 0){
      this.editAssessment(event?.row, event?.value, event?.field);
    }
  }

  editAssessment(row?:any,assessment?: any, field?: any) {
    this._aiGraderService
      .editAssessment(row?.id, row?.name)
      ?.subscribe({
        next: (response: any) => {
          if (
            response?.status ===
