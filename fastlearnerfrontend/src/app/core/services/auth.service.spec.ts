...
describe('AuthService', () => {
  ...
  it('should handle refresh token error', fakeAsync(() => {
    const refreshToken = 'refresh-token';
    cacheServiceSpy.getDataFromCache.and.returnValue(refreshToken);
    ...
    tick(500);
    console.log(cacheServiceSpy);

    expect(cacheServiceSpy.removeFromCache).toHaveBeenCalledWith('token');
    expect(cacheServiceSpy.removeFromCache).toHaveBeenCalledWith('refresh_token');
  }));
