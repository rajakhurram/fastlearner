...
export class CourseService {
  ...
  getQuizData(quiz?: any) {
    const qui: Quiz = {
      ...
      generateAIReport: quiz?.generateAIReport || false,
      reportPrompt: quiz?.reportPrompt || '',
      testType: quiz?.type,
      isAllowedToRetake: quiz?.isAllowedToRetake || false,
      questions: this.getQuizQuestions(quiz.questions, quiz?.type),
    };
    return qui;
  }
      ...
      if (this.validateQuestion(question, isSurvey)) {
        const ques: Question = {
          ...
          answers: isSurvey
            ...
            : this.getQuestionAnswers(question.answers, question.correctAnswer),
        };
