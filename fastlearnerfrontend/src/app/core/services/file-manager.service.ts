...
export class FileManager {
  ...
  public deleteFile(
    ...
    return this._http.delete(`${environment.baseUrl}uploader/`, {
      headers: headers,
      body: body,
    });
  }
  ...
  downloadExcelTemplate(fileName: string = 'template.xlsx'): void {
    const filePath = `assets/templates/${fileName}`;

    this._http.get(filePath, { responseType: 'blob' }).subscribe({
      next: (blob) => {
