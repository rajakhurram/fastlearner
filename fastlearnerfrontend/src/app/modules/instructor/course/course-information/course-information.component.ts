...
import { previewVideoRequiredValidator } from 'src/app/core/utils/preview-video-validator';
...
export class CourseInformationComponent {
  ...
  validateAndContinue(): void {
    ...
    // Force validation for course summaries and tags
    this.validateCourseSummaries();
    this.validateTags();
...
private validateTags(): void {
  ...
  showValidationErrors(): void {
    ...
    if (this.formGroup.get('urlExist').value) {
      errors.push('Course URL is not available');
    }
    if (this.courseTagArray.controls.length === 0) {
