...
describe('NavbarComponent', () => {
  ...
  describe('routeToInstructorWelcomePage', () => {
    it('should navigate to welcome instructor page and reset navbarVisible', fakeAsync(() => {
      const authService = TestBed.inject(AuthService);
      spyOn(authService, 'welcomeInstructor').and.returnValue(of({
        data: {
          welcomeInstructorDashboard: false,
        }
      }));

      // Spy on the component's private _router property
      ...
      expect(document.body.classList.contains('hide-scrollbar')).toBeFalse();
      expect(component.navbarVisible).toBeFalse();
      expect(component['_router'].navigate).toHaveBeenCalledWith(['welcome-instructor']);
    }));
  });
