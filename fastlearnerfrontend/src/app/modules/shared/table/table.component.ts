...
export class TableComponent implements OnChanges {
  ...
  editValue(row: any, field: string, isEnter: boolean) {
    ...
    if (isEnter) {
      if (newValue?.length > 0 && newValue !== row.previousValue) {
        ...
        row[field] = row.previousValue;
        row.editableKey = row.editableKey?.filter((f: string) => f !== field);
      }
    } else {
      row[field] = row.previousValue;
      this.onAction(null, 'edit', row);
    }
  }

  onEditStart(row: any, field: string) {
    row.previousValue = row[field]; // save the old value
  }
