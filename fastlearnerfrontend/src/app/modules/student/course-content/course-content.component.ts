...
export class CourseContentComponent implements OnInit, AfterViewInit {
  ...
  showQuizPlayerWelcomeScreen: boolean = false;
...
setCurrentTopicAndSection(topic: any, section: any) {
  ...
  updateCourseProgressOnReview(section: any, topic: any): void {
    if (!section || !topic) {
      console.warn('Section or topic is missing');
      return;
    }

    // Mark topic as completed locally
    ...
    if (topic.topicType === 'Video') {
      completePayload = {
        ...
        seekTime:
          topic.topicId !== this.currentSelectedTopicId
            ? 0
            : this.getTimeInSec(this.currentVideoTime),
      };
    } else {
    ...
    this._courseService.markTopicComplete(completePayload)?.subscribe({
      ...
      error: (error: any) => {
        topic.isCompleted = false; // revert local completion on failure
        console.error('Error marking topic as complete:', error);
      },
    });
  }
