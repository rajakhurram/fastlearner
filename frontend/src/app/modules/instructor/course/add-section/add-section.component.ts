...
export class AddSectionComponent implements OnInit {
        ...
        if (
          ...
          response?.data?.forEach((topic?: any, index?: any) => {
            ...
            if (topic.topicType == this.typeQuiz) {
              ...
              quiz.randomQuestion = topic?.randomQuestion;
  ...
  addQuestion(topic?: any, questions?: any) {
    ...
    if(this.selectedContentType == this.courseContentType.TEST && topic?.quiz?.randomQuestionType == this.randomQuestionAll){
  ...
  quizValidation(topic?: any) {
    ...
    if (
      ...
      for (let i = 0; i < questions.length && outerLopop == true; i++) {
        ...
        if (
          questions[i].ques != '' &&
          answers.filter((answer: any) => answer.isCorrectAnswer)?.length > 0 &&
          answers.length > 1
        ) {
